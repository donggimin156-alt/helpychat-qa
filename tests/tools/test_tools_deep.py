...
def test_FHC_057_deep_research_happy_path(deep):
    ...
    전제: 로그인 완료 상태
    단계:
      1. LNB > '도구' 탭 클릭 → '심층 조사' 메뉴 클릭 (FHC-057)
      2. 주제 입력 필드에 '날씨' 입력 (FHC-058)
      3. 지시사항 입력 필드에 '대한민국 서울의 2026년 5월의 날씨' 입력 (FHC-059)
      4. [자동 생성] 버튼 클릭 → 로딩 스피너 확인 (FHC-060)
    기대: '심층 조사' 페이지 진입 → 버튼 활성화 → 생성 시작(스피너 표시)
    """
    logger.info("[FHC-057] 심층 조사 해피패스 시작")

    with allure.step("[FHC-057] 심층 조사 페이지 진입 확인"):
        deep.is_tool_page_displayed()

    with allure.step("[FHC-058~059] 주제 및 지시사항 입력"):
        deep.enter_text(DeepPage.TOPIC_INPUT, DeepPage.TOPIC_TEXT)
        deep.enter_text(DeepPage.MESSAGE_INPUT, DeepPage.MESSAGE_TEXT)

    with allure.step("[FHC-060] 생성 버튼 활성화 확인 및 클릭"):
        deep.assert_generate_btn_enabled()
        deep.click_generate()

    with allure.step("[FHC-060] 생성 시작(스피너) 확인"):
        assert deep.is_generating(), "생성이 시작되지 않았습니다"

    logger.info("[FHC-057] 심층 조사 해피패스 — 생성 시작 확인 완료")
...
def test_FHC_057a_deep_research_is_generated(deep):
    ...
    전제: test_FHC_057에서 생성이 시작된 상태 (module fixture 공유)
    단계:
      1. 생성 완료 체크 아이콘 확인 (최대 10분) (FHC-060a)
    기대: 10분 이내 생성 완료
    """
    logger.info("[FHC-057a] 심층 조사 생성 완료 확인 시작")

    with allure.step("[FHC-060a] 생성 완료 확인 (최대 10분)"):
        assert deep.is_generated(timeout=600), "10분 이내 생성 실패"

    logger.info("[FHC-057a] 심층 조사 생성 완료 확인 완료")
...
def test_FHC_061_blank_topic_error(deep_sad):
    ...
    with allure.step("[FHC-061] 주제 공백 입력 후 생성 버튼 클릭"):
    ...
    with allure.step("[FHC-061] 오류 메시지 표시 확인"):
