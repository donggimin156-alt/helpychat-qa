...
def test_quiz_happy_path(quiz):
    ...
    전제: 로그인 완료 상태
    단계:
      1. LNB > '도구' 탭 클릭 → '퀴즈 생성' 메뉴 클릭 (FHC-054)
      2. 문제 유형 '객관식 (단일 선택)', 난이도 '하', 주제 '퀴즈' 입력 (FHC-055)
      3. [자동 생성] 버튼 클릭 → 로딩 스피너 확인 → 생성 완료 (최대 2분) (FHC-056)
    기대: '퀴즈 생성' 페이지 진입 → 버튼 활성화 → 2분 이내 생성 완료
    """
    logger.info("[FHC-054] 퀴즈 생성 해피패스 시작")

    with allure.step("[FHC-054] 퀴즈 생성 페이지 진입 확인"):
        quiz.is_tool_page_displayed()

    with allure.step("[FHC-055] 문제 유형, 난이도, 주제 입력"):
    ...
    with allure.step("[FHC-056] 생성 버튼 활성화 확인 및 클릭"):
        quiz.assert_generate_btn_enabled()
        quiz.click_generate()

    with allure.step("[FHC-056] 생성 시작(스피너) 확인"):
        assert quiz.is_generating(), "생성이 시작되지 않았습니다"

    with allure.step("[FHC-056] 생성 완료 확인 (최대 2분)"):
        assert quiz.is_generated(timeout=120), "2분 이내 퀴즈 생성 실패"

    logger.info("[FHC-054] 퀴즈 생성 해피패스 완료")
