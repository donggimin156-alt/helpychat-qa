# pages/tools/lesson_plan_page.py
# '수업지도안' 도구 전용 Page 클래스
...
class LessonPlanPage(BaseToolPage):
    ...
    # 참고자료 업로드
    FILE_UPLOAD_TEXT = (By.CSS_SELECTOR, "div[data-scope='file-upload'][data-part='dropzone'] p")
    ...
    def select_generation_method(self, method: str = "basic"):
    ...
    def click_upload_area_and_verify(self) -> bool:
        """파일 선택 창 활성화 확인 후 취소로 닫기"""
        VK_ESCAPE  = 0x1B
        WM_COMMAND = 0x0111
        IDCANCEL   = 2
    ...
    def upload_reference(self, file_path: str):
    ...
    @staticmethod
    def _pick_different(options: list, current: str) -> str:
        for opt in options:
            if opt != current:
                return opt
        return options[0]
